...
Cloudflare can still close long-lived responses if the origin does not send data for long enough. The expected healthy behavior for Kanso is:
...
### Owner checklist: App Store Connect rename to Kanso

After merging the repo rebrand slice, update App Store Connect metadata separately:

1. Production app (`app.kanso`): rename listing from `Hermes Agent Mobile` → `Kanso`.
2. Branch TestFlight app (`app.kanso.branch`): rename listing from `Hermes Agent Branch` → `Kanso Branch`.
3. Update TestFlight/review notes and any metadata copy that still says the old app name.
4. Upload a build and confirm TestFlight shows **Kanso** / **Kanso Branch** on the home screen after processing.
...
to the side-by-side **Kanso Branch** internal TestFlight app. This is a TestFlight
upload, **not** a Git push. Never merge, Git push, or upload the production
`app.kanso` TestFlight app unless the owner explicitly asks.

Branch TestFlight app identity:

- App Store Connect app name: `Kanso Branch`
- Main bundle ID: `app.kanso.branch`
- Share extension bundle ID: `app.kanso.branch.shareextension`
- Live Activity widget bundle ID: `app.kanso.branch.liveactivitywidget`
- Display name: `Kanso Branch`
- App group: `group.app.kanso.branch`
