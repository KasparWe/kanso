...
/// deep-link router. An intent writes a `kanso://…` URL here; `ContentView` observes
...
final class AppIntentRouter {
    ...
    /// pass the optional `KansoDeepLink` builders without unwrapping.
...
/// "New Chat" — opens Kanso on the New Chat composer, mirroring the in-app "+" button
/// (no server session is created until the first message). Available to the Action button,
/// Shortcuts, Spotlight, and Siri via `KansoShortcuts`.
struct NewChatIntent: AppIntent {
    static var title: LocalizedStringResource = "New Chat"
    static var description = IntentDescription("Open Kanso on a new, empty chat.")
    ...
    func perform() async throws -> some IntentResult {
        AppIntentRouter.shared.requestDeepLink(KansoDeepLink.newChatURL)
        return .result()
    }
}

/// "New Chat with Voice" — opens Kanso on the New Chat composer and auto-starts voice
...
struct NewChatVoiceIntent: AppIntent {
    ...
    static var description = IntentDescription("Open Kanso on a new chat and start voice dictation.")
    ...
    @MainActor
    func perform() async throws -> some IntentResult {
        AppIntentRouter.shared.requestDeepLink(KansoDeepLink.newChatVoiceURL)
        return .result()
    }
}

/// "New Chat in <Profile>" — opens Kanso on a new chat pinned to a specific server profile
...
struct NewChatInProfileIntent: AppIntent {
    ...
    static var description = IntentDescription("Open Kanso on a new chat pinned to a specific profile.")
    ...
    func perform() async throws -> some IntentResult {
        AppIntentRouter.shared.requestDeepLink(
            KansoDeepLink.newChatInProfileURL(profileName: profile.id)
        )
        return .result()
    }
}

/// Registers Kanso's App Shortcuts. iOS discovers this conformance automatically at build
...
struct KansoShortcuts: AppShortcutsProvider {
