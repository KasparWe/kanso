...
struct ContentView: View {
    ...
    private func handleOpenURL(_ url: URL) {
        ...
        if KansoDeepLink.isNewChatVoiceURL(url) {
        ...
        if KansoDeepLink.isNewChatInProfileURL(url) {
            pendingNewChatRequest = NewChatRequest(
                profileName: KansoDeepLink.profileName(fromNewChatInProfile: url)
            )
            return
        }

        if KansoDeepLink.isNewChatURL(url) {
            pendingNewChatRequest = NewChatRequest(autoStartsVoiceInput: false)
            return
        }

        if let sessionID = KansoDeepLink.sessionID(from: url) {
