                    ...
                    if let settingsURL = URL(string: UIApplication.openSettingsURLString) {
                        Link(destination: settingsURL) {
                            SettingsAccessoryRow(
                                title: String(localized: "Open Kanso Settings"),
                                systemImage: "gearshape",
                                accessorySystemImage: "arrow.up.forward"
                            )
                        }
                        .buttonStyle(.plain)
                        .accessibilityLabel("Open Kanso Settings")
                    }

                    SettingsFootnote(String(localized: "Run Kanso actions like New Chat from Siri, Spotlight, the Lock Screen, or the iPhone Action button. Open Kanso Settings to manage its Siri & Search options. To assign an action to the Action button, open the iOS Settings app, choose Action Button, then Shortcut, and pick a Kanso action."))
                }
...
private struct HeaderLogoColorSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            KansoHeaderLogo(selectedColor: HeaderLogoColor.color(for: selectedHex))
