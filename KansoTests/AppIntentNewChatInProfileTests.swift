...
final class AppIntentNewChatInProfileTests: XCTestCase {
    ...
    func testNewChatInProfileURLUsesProfileHostAndCarriesName() throws {
        let url = try XCTUnwrap(KansoDeepLink.newChatInProfileURL(profileName: "dev"))
        XCTAssertEqual(url.scheme, KansoDeepLink.scheme)
        XCTAssertEqual(url.host, KansoDeepLink.newChatInProfileHost)
        XCTAssertEqual(KansoDeepLink.profileName(fromNewChatInProfile: url), "dev")
    }

    func testBlankProfileNameProducesNoURL() {
        XCTAssertNil(KansoDeepLink.newChatInProfileURL(profileName: ""))
        XCTAssertNil(KansoDeepLink.newChatInProfileURL(profileName: "   "))
    }

    func testProfileNameWithSpacesAndUnicodeRoundTrips() throws {
        let name = "Work Café 工作"
        let url = try XCTUnwrap(KansoDeepLink.newChatInProfileURL(profileName: name))
        XCTAssertEqual(KansoDeepLink.profileName(fromNewChatInProfile: url), name)
    }
    ...
    func testIsNewChatInProfileURLIsCaseInsensitiveOnHost() throws {
        let url = try XCTUnwrap(URL(string: "\(KansoDeepLink.scheme)://New-Chat-Profile?profile=dev"))
        XCTAssertTrue(KansoDeepLink.isNewChatInProfileURL(url))
    }

    func testProfileNameNilWhenQueryMissing() throws {
        let url = try XCTUnwrap(URL(string: "\(KansoDeepLink.scheme)://new-chat-profile"))
        XCTAssertNil(KansoDeepLink.profileName(fromNewChatInProfile: url))
    }

    func testForeignSchemeIsNotAProfileURL() throws {
        let url = try XCTUnwrap(URL(string: "https://new-chat-profile?profile=dev"))
        XCTAssertFalse(KansoDeepLink.isNewChatInProfileURL(url))
    }

    // MARK: - Non-aliasing with the other new-chat links

    func testProfileURLDoesNotAliasPlainVoiceOrSession() throws {
        let profileURL = try XCTUnwrap(KansoDeepLink.newChatInProfileURL(profileName: "dev"))
        XCTAssertFalse(KansoDeepLink.isNewChatURL(profileURL))
        XCTAssertFalse(KansoDeepLink.isNewChatVoiceURL(profileURL))
        XCTAssertNil(KansoDeepLink.sessionID(from: profileURL))
    }

    func testPlainVoiceAndSessionURLsAreNotProfileURLs() throws {
        let plain = try XCTUnwrap(KansoDeepLink.newChatURL)
        let voice = try XCTUnwrap(KansoDeepLink.newChatVoiceURL)
        let session = try XCTUnwrap(KansoDeepLink.sessionURL(sessionID: "abc123"))
        XCTAssertFalse(KansoDeepLink.isNewChatInProfileURL(plain))
        XCTAssertFalse(KansoDeepLink.isNewChatInProfileURL(voice))
        XCTAssertFalse(KansoDeepLink.isNewChatInProfileURL(session))
        XCTAssertNil(KansoDeepLink.profileName(fromNewChatInProfile: plain))
    }
    ...
    func testIntentQueuesTheProfileDeepLink() async throws {
        ...
        XCTAssertEqual(
            ...
            KansoDeepLink.newChatInProfileURL(profileName: "dev")
        )
    }
