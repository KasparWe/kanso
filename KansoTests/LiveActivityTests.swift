...
final class LiveActivityTests: XCTestCase {
    ...
    func testBuildsAndParsesSessionDeepLink() throws {
        let url = try XCTUnwrap(KansoDeepLink.sessionURL(sessionID: "session-abc"))
        let scheme = KansoDeepLink.scheme

        XCTAssertEqual(url.scheme, scheme)
        XCTAssertEqual(url.host, "session")
        XCTAssertEqual(KansoDeepLink.sessionID(from: url), "session-abc")
        XCTAssertEqual(KansoDeepLink.sessionID(from: URL(string: "\(scheme)://session/session-xyz")!), "session-xyz")
        XCTAssertNil(KansoDeepLink.sessionID(from: HermesShareDraft.openURL))
    }

    func testSessionDeepLinkURLPercentEncodesSessionID() throws {
        let sessionID = "session & /?=✓"
        let url = try XCTUnwrap(KansoDeepLink.sessionURL(sessionID: sessionID))
